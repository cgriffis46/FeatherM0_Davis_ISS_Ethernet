...
void DavisRFM69::initialize(byte freqBand) {
  const byte CONFIG[][2] = {
    ...
    /* 0x0B */
    { REG_AFCCTRL, RF_AFCLOWBETA_OFF },                                               // TODO: Should use LOWBETA_ON, but having trouble getting it working

//    {REG_LISTEN1,RF_LISTEN1_RESOL_4100|RF_LISTEN1_CRITERIA_RSSIANDSYNC|RF_LISTEN1_END_01},
//    {REG_LISTEN2,RF_LISTEN2_COEFIDLE_VALUE},
//    {REG_LISTEN3,RF_LISTEN3_COEFRX_VALUE},
 ...
 /* 0x3b REG_AUTOMODES - Automatic modes are not used in this implementation. */
   // {REG_AUTOMODES,RF_AUTOMODES_INTERMEDIATE_STANDBY|RF_AUTOMODES_EXIT_FIFOEMPTY|RF_AUTOMODES_ENTER_CRCOK},
...
//        vTaskSetTimeOutState(&xTimeOut);
 ...
 //       vTaskSetTimeOutState(&xStationTimeOut[i]);
        ...
        //setChannel(stations[i].channel);
        setChannel(stations[i].channel);
      } else if (xTaskGetTickCount()-stations[i].syncBegan > DISCOVERY_STEP/portTICK_PERIOD_US) {
...
#ifdef DAVISRFM69_DEBUG
        byte p = difftime(xTaskGetTickCount(), stations[i].syncBegan) / (DISCOVERY_STEP / 100);
        ...
        if (stations[i].progress != p) {
          ...
          //xTaskNotify(xWatchdogTaskHandle,0x01,eSetBits);// feed watchdog
        }
#endif
        break;
      }
...
void DavisRFM69::handleRadioInt() {
  ...
  TickType_t lastRx = PayloadReadyTicks > 0 ? PayloadReadyTicks: xTaskGetTickCount();
...
//  Serial.print("IntWait:"); Serial.print(ModeReady);
...
//  Serial.println(ModeReady);
  ...
  if (calcCrc == rxCrc && rxCrc != 0) {
    ...
    stations[stIx].earlyAmt = lastRx-stations[stIx].recvBegan;
...
//       while(!ModeReady);
    ...
    case RF69_MODE_LISTEN:
      writeReg(REG_OPMODE, (readReg(REG_OPMODE) & 0x9F) | RF_OPMODE_LISTEN_ON );
      break;
    default: return;
  }
...
void DavisRFM69::unselect(bool FromISR) {
  //digitalWrite(_slaveSelectPin, HIGH);
